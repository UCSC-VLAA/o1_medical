...
def bleu_score(prediction: str, references: list[str], n_gram=1) -> float:
    try:
        return bleu(prediction, references, n_gram = n_gram).item()
    except:
        return torch.tensor(0.0)


def RougeL_score(prediction: str, references: list[str]) -> float:
    rouge = RougeL()
    rouge.update((prediction, references))
    return rouge.compute()
def RougeN_score(prediction: str, references: list[str], n_gram = 1) -> float:
    rouge = RougeN(ngram=n_gram)
    rouge.update((prediction, references))
    return rouge.compute()
...
def f1_score_precision_recall(prediction: str, answers: list[str]) -> float:
    ...
    return max([_f1_score(prediction, answer) for answer in answers], key=lambda x: x[0])
