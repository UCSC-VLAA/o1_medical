...
def compute_averaged_precision(
    confusion_matrix: np.ndarray, average: str = "macro"
) -> float:
    assert average in ["macro"]
    precisions = []
    for i in range(confusion_matrix.shape[0]):
        precisions.append(compute_precision(confusion_matrix, idx=i))
    return np.array(precisions).mean()

def compute_averaged_recall(
    confusion_matrix: np.ndarray, average: str = "macro"
) -> float:
    assert average in ["macro"]
    recalls = []
    for i in range(confusion_matrix.shape[0]):
        recalls.append(compute_recall(confusion_matrix, idx=i))
