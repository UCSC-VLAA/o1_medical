...
class RecorderBase:
    ...
    def get_sampling(self):
        events = list(self.get_events("sampling"))
        for x in events:
            print(x.data.keys())
        sampled = [x.data['sampled'] for x in events]
        expected = [x.data['expected'] for x in events]
...
class LocalAutoRecorder(LocalRecorder):
    def get_scores(self):
        events = self.get_events("metrics")
        scores = {}
        for e in events:
