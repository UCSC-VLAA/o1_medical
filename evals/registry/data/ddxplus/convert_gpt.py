...
output_dir = '/workspace/evals/evals/registry/data/ddxplus'
...
with open('/workspace/datasets/MedS-Bench/Diagnosis/task130_DDXPlus_text_classification_test.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for idx, instance in enumerate(few_shot_instances, 1):
    ...
    assert output_text.startswith(diagnosis_prefix)
    output_text = output_text[len(diagnosis_prefix):].strip()
    diagnosis = output_text.rstrip('.')
        
    
    few_shot_case += f"Case{idx}: Input: {input_text}, Output: {diagnosis}\n"
...
for idx, instance in enumerate(remaining_instances):
    ...
    output_dict = {
        "input": [
            {
                "role": "user",
                "content": f"{prompt}{input_text} {data['Definition'][0]}",
            }
        ],
        "ideal": diagnosis
    }
