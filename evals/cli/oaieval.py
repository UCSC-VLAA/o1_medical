...
def get_parser() -> argparse.ArgumentParser:
    ...
    #add argument for switch api key
...
def run(args: OaiEvalArguments, registry: Optional[Registry] = None) -> str:
    ...
    record_path = (
        f"./evallogs/{args.eval}_{time}_{args.completion_fn}.jsonl"
...
def build_recorder(
    ...
    if args.local_run:
        return evals.record.LocalAutoRecorder(record_path, run_spec=run_spec)
    ...
    return evals.record.AutoRecorder(
        record_path,
        run_spec=run_spec,
    )
...
def add_token_usage_to_result(result: dict[str, Any], recorder: RecorderBase) -> None:
    ...
    logger.info(usage_events)
    if usage_events:
        # Sum up the usage of all samples
        total_usage = {}
        for u in usage_events:
            for key, value in u.items():
                if isinstance(value, (int, float)):
                    total_usage[key] = total_usage.get(key, 0) + (value if value is not None else 0)
                elif hasattr(value, '__dict__'):
                    # Handle objects with attributes
                    for subkey, subvalue in value.__dict__.items():
                        fullkey = f"{key}_{subkey}"
                        total_usage[fullkey] = total_usage.get(fullkey, 0) + (subvalue if subvalue is not None else 0)
                    ...
                    logger.warning(f"Skipping key {key} with unsupported value type: {type(value)}")
